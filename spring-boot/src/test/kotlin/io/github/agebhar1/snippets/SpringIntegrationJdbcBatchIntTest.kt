...
@JdbcTest
@Testcontainers
class SpringIntegrationJdbcBatchIntTest(@Autowired val jdbcTemplate: JdbcTemplate, @Autowired val messagingTemplate: MessagingTemplate) {
    ...
    fun `list of tuples should be present in database table`() {
        ...
        val message = MessageBuilder
                .withPayload(listOf(
                        listOf(1, "1st message"),
                        listOf(2, "2nd message"),
                        listOf(3, "3rd message")
                ))
        ...
        assertThat(countRowsInTableWhere(jdbcTemplate, "message", "messageId = '$messageId'")).isEqualTo(3)
    ...
    fun `list of tuples should be updated in database table`() {
        ...
        val sndMessage = MessageBuilder
                .withPayload(listOf(
                        listOf(1, "message one"),
                        listOf(2, "message two"),
                        listOf(3, "message three")
                ))
                .setHeader("messageId", messageId)
                .build()
        ...
        assertThat(countRowsInTableWhere(jdbcTemplate, "message", "payload IN ('message one', 'message two', 'message three')")).isEqualTo(3)
    ...
    class Configuration {
        ...
        @Bean
        fun messagingTemplate() = MessagingTemplate(input())

        @Bean
        fun flow(jdbcTemplate: JdbcTemplate) = integrationFlow(input()) {
