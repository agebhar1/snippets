...
int middleware_write(const char *queue) {
    MessageHdr hdr = {.opcode = WRITE, .size = middleware_buffer_size};
    strncpy(hdr.queue, queue, sizeof(hdr.queue) - 1);
    hdr.queue[sizeof(hdr.queue) - 1] = '\0';
    ...
    if (hdr.size > 0) {
        if (write(middleware_fd, middleware_buffer, middleware_buffer_size) != (ssize_t) middleware_buffer_size) {
            return -1;
        }
    }

    (void) opentelemetry_write(queue, middleware_buffer, middleware_buffer_size);
