#include <getopt.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <tgmath.h>
#include <time.h>
#include <unistd.h>

#include "middleware.h"

typedef struct {
    char *r;
    char **w;
    struct timespec duration;
    unsigned int len;
    bool getline;
} Configuration;

static Configuration get_config(const int argc, char *const argv[]) {
    Configuration config = {
        .r = nullptr,
        .w = nullptr,
        .duration = {
            .tv_sec = 0,
            .tv_nsec = 0
        },
        .len = 0,
        .getline = true
    };

    int opt;
    while ((opt = getopt(argc, argv, "fd:r:w:")) != -1) {
        switch (opt) {
            case 'f':
                config.getline = false;
                break;
            case 'd':
                double duration = atof(optarg);
                if (duration < 0.0) {
                    fprintf(stderr, "WARNING: option 'd'(uration) must be >= 0.0\n");
                    break;
                }
                config.duration.tv_sec = (time_t) duration;
                config.duration.tv_nsec = (long) ((duration - floor(duration)) * 1e9);
                break;
            case 'r':
                if (config.r == nullptr) {
                    config.r = strdup(optarg);
                } else {
                    fprintf(stderr, "WARNING: option 'r' already set by '%s', '%s' ignored\n", config.r, optarg);
                }
                break;
            case 'w':
                config.w = realloc(config.w, (config.len + 1) * sizeof(char *));
                config.w[config.len] = strdup(optarg);
                config.len++;
                break;
            default:
                fprintf(stderr, "Usage: %s [-r queue] [-w queue]+ [-d duration] [-f]\n", argv[0]);
                exit(EXIT_FAILURE);
        }
    }

    if (config.r != nullptr || config.w != nullptr) {
        printf("%s", config.r == nullptr ? (config.getline ? "(stdin [getline])" : "(stdin [read])") : config.r);
        if (config.w != nullptr) {
            printf(" -> ");
            for (unsigned int i = 0; i < (config.len - 1); i++) {
                printf("%s | ", config.w[i]);
            }
            printf("%s", config.w[config.len - 1]);
        }
        printf("\n\n");
    }

    return config;
}

static void free_config(Configuration *config) {
    if (config->r != nullptr) {
        free(config->r);
        config->r = nullptr;
    }
    if (config->w != nullptr) {
        for (unsigned short i = 0; i < config->len; i++) {
            free(config->w[i]);
        }
        free(config->w);
        config->w = nullptr;
    }
}

int main(const int argc, char *const argv[]) {
    Configuration config = get_config(argc, argv);
    if (config.r == nullptr && config.w == nullptr) {
        goto done;
    }

    if (middleware_connect() == -1) {
        printf("failed to connect to middleware\n");
        goto done;
    }

    char *line = nullptr;
    size_t length = 0;

    while (1) {
        // read
        if (config.r == nullptr) {
            const ssize_t size = config.getline
                                     ? getline(&line, &length, stdin)
                                     : read(STDIN_FILENO, middleware_buffer, sizeof(middleware_buffer));
            if (size == -1 || size == 0) {
                break;
            }
            if (config.getline) {
                if (size > (ssize_t) sizeof(middleware_buffer)) {
                    printf("middleware buffer exhausted: required: %ld, capacity: %ld\n", size,
                           sizeof(middleware_buffer));
                    break;
                }
                memcpy(middleware_buffer, line, size);
            }

            middleware_buffer_size = size;
            if (middleware_buffer[size - 1] == '\n') {
                middleware_buffer[size - 1] = '\0';
                middleware_buffer_size -= 1;
            }
#ifndef NDEBUG
            printf("read from stdin => '%s' (size: %ld)\n", (const char *) middleware_buffer, middleware_buffer_size);
#endif
        } else {
            if (middleware_read(config.r) == -1) {
                printf("failed to read from middleware\n");
                break;
            }
            printf("read from queue '%s' => '%s'\n", config.r, (const char *) middleware_buffer);
        }

        // business process
        if (config.r != nullptr) {
            printf("[ processing ... (%ld.%09lds) ]\n", config.duration.tv_sec, config.duration.tv_nsec);
            nanosleep(&config.duration, nullptr);
        }

        // write
        for (unsigned short i = 0; i < config.len; i++) {
            if (middleware_write(config.w[i]) == -1) {
                printf("failed to write to middleware\n");
                break;
            }
        }
    }

    middleware_disconnect();

done:
    if (line != nullptr) { free(line); }
    free_config(&config);

    return EXIT_SUCCESS;
}
